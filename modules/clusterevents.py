...
async def asyncschedholidayevent(startdate, enddate):
    if not await asyncgetnexteventinfo():
        einfo = dbquery(
            "SELECT * FROM autoevents WHERE title = 'Holiday'", fmt="dict", fetch="one"
        )
        dbupdate(
        ...
        log.log(
            "EVENTS", f"Scheduling next Evolution Weekend Event {startdate} - {enddate}"
        )
    else:
        log.log(
            "EVENTS",
...
async def asynciseventrebootday():
    startday = await db.fetchone(
        f"SELECT title FROM events WHERE starttime = '{Now(fmt='dtd')}'"
    )
    endday = await db.fetchone(
        f"SELECT title FROM events WHERE endtime = '{Now(fmt='dtd')}'"
    )
    if startday:
...
async def asynciseventtime():
    inevent = await db.fetchone(
        f"""SELECT * FROM events WHERE completed = 0 AND (starttime < '{Now(fmt="dtd")}' OR starttime = '{Now(fmt="dtd")}')"""
    )
    if inevent:
...
def iseventtime():
    inevent = dbquery(
        "SELECT * FROM events WHERE completed = 0 AND (starttime < '%s' OR starttime = '%s')"
...
async def asyncgetcurrenteventext():
    if await asynciseventtime():
        inevent = await db.fetchone(
            "SELECT cfgfilesuffix FROM events WHERE completed = 0 AND (starttime < '%s' OR starttime = '%s')"
            % (Now(fmt="dtd"), Now(fmt="dtd"))
        )
        if inevent:
...
def getcurrenteventtitle():
    if iseventtime():
        inevent = dbquery(
            "SELECT title FROM events WHERE completed = 0 AND (starttime < '%s' OR starttime = '%s')"
            % (Now(fmt="dtd"), Now(fmt="dtd")),
            fetch="one",
        )
        if inevent:
...
async def asyncgetlasteventinfo():
    inevent = await db.fetchone(
        f"SELECT * FROM events WHERE completed = 1 ORDER BY id DESC"
    )
    return inevent


async def asyncgetnexteventinfo():
    inevent = await db.fetchall(
        f"""SELECT * FROM events WHERE completed = 0 AND starttime > '{Now(fmt="dtd")}' or starttime = '{Now(fmt="dtd")}' ORDER BY starttime ASC"""
    )
    if inevent:
...
async def asyncstartserverevent(inst):
    await db.update(
        f"UPDATE instances SET inevent = '{await asyncgetcurrenteventext()}' WHERE name = '{inst}'"
    )
...
async def asynccheckifeventover():
    curevent = await db.fetchone(
        f"""SELECT * FROM events WHERE completed = 0 AND (endtime < '{Now(fmt="dtd")}' OR endtime = '{Now(fmt="dtd")}') ORDER BY endtime ASC"""
    )
...
async def asynccheckifeventstart():
    curevent = await db.fetchone(
        f"""SELECT * FROM events WHERE completed = 0 AND announced = False AND (starttime < '{Now(fmt="dtd")}' OR starttime = '{Now(fmt="dtd")}') ORDER BY endtime ASC"""
    )
    if curevent and await asynciseventtime():
        ...
        await db.update(
            f"""UPDATE events SET announced = True WHERE id = '{curevent["id"]}'"""
        )
...
async def asynceventwatcher():
    ...
    for inst in await globalvar.getlist("allinstances"):
        if await asynciseventtime() and await asynccurrentserverevent(inst) == "0":
            ...
            not await asynciseventtime() and await asynccurrentserverevent(inst) != "0"
        ):
