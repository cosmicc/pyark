...
async def addmessage(mtype, messageid):
    ...
    await db.update(
        "INSERT INTO messagetracker (messagetype, messageid) VALUES ('%s', '%s')"
        % (mtype, messageid)
    )
...
async def gettip():
    tip = await db.fetchone(
        "SELECT * FROM tips WHERE active = True ORDER BY count ASC, random()"
    )
    await db.update(
        "UPDATE tips set count = %s WHERE id = %s" % (int(tip["count"]) + 1, tip["id"])
    )
    return tip["tip"]
...
async def savediscordtodb(author):
    didexists = await db.fetchone(
        "SELECT * FROM discordnames WHERE discordname = '%s'"
        % (str(author).replace("'", ""),)
    )
    if not didexists:
        await db.update(
            "INSERT INTO discordnames (discordname) VALUES ('%s')"
            % (str(author).replace("'", ""),)
        )
...
def pyarkbot():
    ...
    async def on_ready():
        ...
        log.log(
            "SUCCESS", f"Discord logged in as {client.user.name} id {client.user.id}"
        )
    ...
    async def on_member_join(member):
        ...
        fmt = (
            fmt
            + "Type **`!servers`** for links and status of the servers\nType **`!mods`** for a link to the mod collection\n**`!help`** for all the other commands\n\n"
        )
        fmt = (
            fmt
            + "More help can be found with pinned messages in **#help**\nDont be afraid to ask for help in discord!"
        )
        embed = discord.Embed(
            title="Welcome to the Galaxy Cluster Ultimate Extinction Core Server Discord!",
            description=fmt,
            color=HELP_COLOR,
        )
    ...
    async def on_command_error(ctx, error):
        try:
            if isinstance(error, commands.CommandNotFound):
                log.warning(
                    f"Invalid discord command [{ctx.message.content}] sent from [{ctx.message.author}]"
                )
                ...
                log.warning(
                    f"discord bot error for {ctx.message.author}: {ctx.message.content} - {error}"
                )
        except:
    ...
    async def clearmessages(ntype):
        ...
        msgs = await db.fetchall(
            f"SELECT messageid FROM messagetracker WHERE messagetype = '{ntype}'"
        )
        if msgs:
            for msgid in msgs:
                try:
                    ...
                    await db.update(
                        f"""DELETE from messagetracker WHERE messageid = '{msgid["messageid"]}'"""
                    )
    ...
    async def taskchecker():
        ...
        while not client.is_closed():
            try:
                ...
                if (
                    Now(fmt="dt") - await getlastannounce("lastlottoannounce")
                    > timedelta(hours=7)
                    and await asyncisinlottery()
                ):
                    linfo = await db.fetchone(
                        "SELECT * FROM lotteryinfo WHERE completed = False"
                    )
                    log.log("LOTTO", "Announcing running lottery in discord")
                    embed = discord.Embed(
                        title=f"A lottery is currently running!", color=INFO_COLOR
                    )
                    embed.set_author(
                        name="Galaxy Cluster Reward Point Lottery",
                        icon_url="https://blacklabelagency.com/wp-content/uploads/2017/08/money-icon.png",
                    )
                    embed.add_field(
                        name=f"Current lottery is up to **{linfo['payout']} Points**",
                        value=f"**{linfo['players'] + 1}** Players have entered into this lottery so far\nLottery ends in **{elapsedTime(datetimeto(linfo['startdate'] + timedelta(hours=linfo['days']), fmt='epoch'),Now())}**\n\n**`!lotto enter`** to join the lottery\n**`!points`** for more information\n**`!winners`** for recent results",
                        inline=True,
                    )
                ...
                if (
                    Now(fmt="dt") - await getlastannounce("lasteventannounce")
                    > timedelta(hours=12)
                    and await asynciseventtime()
                ):
                    log.info("Announcing running event in discord")
                    event = await asyncgetcurrenteventinfo()
                    embed = discord.Embed(
                        title=f"An event is running!", color=INFO_COLOR
                    )
                    embed.set_author(
                        name="Galaxy Cluster Server Events",
                        icon_url="https://library.kissclipart.com/20180903/ueq/kissclipart-party-emoji-clipart-party-popper-emoji-aa28695001083d98.png",
                    )
                    embed.add_field(
                        name=f"The {event[4]} Event is live on all servers",
                        value=f"{event[5]}\nEvent ends in {elapsedTime(Now(), datetimeto(d2dt_maint(event[2]), fmt='epoch'))} ",
                        inline=False,
                    )
                    msg = await generalchat.send(embed=embed)
                ...
                if Now(fmt="dt") - await getlastannounce("lasttipannounce") > timedelta(
                    hours=4
                ):
                ...
                if cbuff:
                    for each in cbuff:
                        if each[0] == "ALERT":
                            ...
                            await setlastannounce("lastlottoannounce", Now(fmt="dt"))
                            embed = discord.Embed(
                                title=f"A new Lottery has started!", color=INFO_COLOR
                            )
                            embed.set_author(
                                name="Galaxy Cluster Reward Points Lottery",
                                icon_url="https://blacklabelagency.com/wp-content/uploads/2017/08/money-icon.png",
                            )
                            embed.add_field(
                                name=f"Starting Pot: {each[2]} Points",
                                value=f"It costs **10 Points** to join a lottery, lottery winnings grow as more join\nLottery will end in **{each[1]}**\n**`!lotto enter`** to join this lottery\n**`!points`** for more information\n**`!winners`** for recent results",
                                inline=False,
                            )
                            msg = await generalchat.send(embed=embed)
                            await clearmessages("lotterymessage")
                            ...
                            embed = discord.Embed(
                                title=f"The current Lottery has ended", color=INFO_COLOR
                            )
                            embed.set_author(
                                name="Galaxy Cluster Reward Points Lottery",
                                icon_url="https://blacklabelagency.com/wp-content/uploads/2017/08/money-icon.png",
                            ...
                            embed.add_field(
                                name=f"Congratulations to {each[2].upper()} winning **{each[1]}** Points",
                                value=f"Next lottery will start in **1** hour\n**`!points`** for more information\n**`!winners`** for recent results",
                                inline=False,
                            )
                            msg = await generalchat.send(embed=embed)
                            await clearmessages("lotterymessage")
                            ...
                            event = await asyncgetcurrenteventinfo()
                            embed = discord.Embed(
                                title=f"A new event is starting!", color=INFO_COLOR
                            )
                            embed.set_author(
                                name="Galaxy Cluster Server Events",
                                icon_url="https://library.kissclipart.com/20180903/ueq/kissclipart-party-emoji-clipart-party-popper-emoji-aa28695001083d98.png",
                            ...
                            embed = discord.Embed(
                                title=f"The current event is ending!", color=INFO_COLOR
                            )
                            embed.set_author(
                                name="Galaxy Cluster Server Events",
                                icon_url="https://library.kissclipart.com/20180903/ueq/kissclipart-party-emoji-clipart-party-popper-emoji-aa28695001083d98.png",
                            ...
                            embed.add_field(
                                name=f"The {event[4]} Event is ending on all servers",
                                value=f"Next event **{nevent[4]}** starts in **{elapsedTime(Now(), datetimeto(d2dt_maint(nevent[2]), fmt='epoch'))}**",
                                inline=False,
                            )
                            msg = await generalchat.send(embed=embed)
                            await clearmessages("eventmessage")
                            ...
                            if Now(fmt="dt") - await getlastannounce(
                                "lastupdateannounce"
                            ) > timedelta(minutes=15):
                                await setlastannounce(
                                    "lastupdateannounce", Now(fmt="dt")
                                )
                                embed = discord.Embed(
                                    title=f"A New Update has been released!",
                                    color=INFO_COLOR,
                                )
                                embed.set_author(
                                    name="ARK Updater for Galaxy Cluster Servers",
                                    icon_url="https://patchbot.io/images/games/ark_sm.png",
                                )
                                embed.add_field(
                                    name=f"Update Reason: {each[2]}",
                                    value=f"{each[1]}\n\nAny applicable servers will begin a **30 min** restart countdown now",
                                    inline=False,
                                )
    ...
    def is_linked(ctx):
        player = dbquery(
            "SELECT * from players WHERE discordid = '%s'"
            % (str(ctx.message.author).lower(),),
            fetch="one",
        )
    ...
    def is_admin(ctx):
        player = dbquery(
            "SELECT steamid from players WHERE discordid = '%s'"
            % (str(ctx.message.author).lower(),),
            fetch="one",
        )
        if player is not None:
            ...
            log.warning(
                f"admin command {ctx.message.content} was rejected for {ctx.message.author}"
            )
            return False
    ...
    def logcommand(ctx):
        ...
        log.log(
            "CMD",
            f"Responding to [{ctx.message.content}] request from [{ctx.message.author}] in [#{dchan}]",
        )
        return True
    ...
    async def messagesend(ctx, embed, allowgeneral=False, reject=True, adminonly=False):
        try:
            if type(ctx.message.channel) == discord.channel.DMChannel:
                ...
                not allowgeneral and str(ctx.message.channel) == "general-chat"
            ):
                role = str(
                    discord.utils.get(ctx.message.author.roles, name="Cluster Admin")
                )
    ...
    async def serversinfo(ctx, refresher=False):
        ...
        for instt in dbsvr:
            if int(instt[9]) == 0:
                ...
                onl = "ONLINE"
                flast = await db.fetchall(
                    f"SELECT * FROM players WHERE server = '{instt[0]}' AND online = True"
                )
                pcnt = len(flast)
            embed.add_field(
                name=f"Server {instt[0].capitalize()} is  **{onl}**  Players ({pcnt}/50)",
                value=f"Hostname: {instt[23]}\nSteam: {instt[15]}\nArkServers: {instt[16]}\nBattleMetrics: {instt[17]}",
                inline=False,
            )
        if refresher:
    ...
    async def serverrules(ctx):
        embed = discord.Embed(
            title="Current Galaxy Cluster Server Rules:", color=INFO_COLOR
        )
        ...
        embed.add_field(
            name=f"If unsure about anything, ever, ask in Discord.  Ignorance is not an excuse.",
            value=msg,
            inline=True,
        )
        await messagesend(ctx, embed, allowgeneral=True, reject=False)
    ...
    async def _help(ctx):
        ...
        msg = (
            msg
            + "**`!servers`**  - Status and links to all the servers in the cluster\n"
        )
        msg = (
            msg + "**`!rates`**  - Current ARK game rates for all the cluster servers\n"
        )
        msg = msg + "**`!rules`**  - List of all the cluster rules\n"
        msg = msg + "**`!events`**  - Current and upcomming cluster events\n"
        msg = (
            msg
            + "**`!decay`**  - Cluster dino & structure decay rates and expire timers\n"
        )
        msg = (
            msg + "**`!who`**  - List all players currently online on all the servers\n"
        )
        msg = (
            msg
            + "**`!timeleft <server>`**  - How much time left in server restart countdown\n"
        )
        msg = msg + "**`!myinfo`**  - Your in-game player information\n"
        msg = (
            msg
            + "**`!expire`**  - Your in-game experation timers and time left before dino/structure decay\n"
        )
        msg = msg + "**`!ec`**  - Links to more Extinction Core Mod information\n"
        msg = (
            msg
            + "**`!points`**  - More information about the Galaxy Cluster points system\n"
        )
        msg = (
            msg
            + "**`!linkme <code>`**  - Link your discord account to your in-game player with <code> from typing !linkme in-game\n"
        )
        msg = (
            msg
            + "**`!kickme`**  - Kick your player from the server it was on so you don't have to wait\n"
        )
        ...
        msg = (
            msg
            + "**`!lastseen <playername>`**  - Show the last time a player was online in-game\n"
        )
        msg = (
            msg
            + "**`!lastwipe <server>`**  - Show the last time a wild dino wipe was performed\n"
        )
        msg = (
            msg
            + "**`!lastrestart <server>`**  - Show the last time the server was restarted and why\n"
        )
        msg = (
            msg
            + "**`!myhome`**  - Show what your current Home server is set to (where all your points go)\n"
        )
        msg = msg + "**`!newest`**  - List the last 5 newest players to the cluster\n"
        msg = (
            msg + "**`!topplayed`**  - List the top 10 players with the most playtime\n"
        )
        ...
        for eachinst in await asyncgetinstancelist():
            msg = msg + f"  **`{eachinst}`**,"
        msg = msg + "\n\n"
        msg = (
            msg
            + "Commands can be privately messaged directly to the bot or in the **#bot-channel**"
        )

        embed = discord.Embed(
            title="Galaxy Custom Bot Commands:", description=msg, color=HELP_COLOR
        )
        await ctx.message.author.send(embed=embed)
        if (
            type(ctx.message.channel) != discord.channel.DMChannel
            and str(ctx.message.channel) != "bot-channel"
        ):
            await ctx.message.delete()
    ...
    async def _primordial(ctx):
        pplayer = await db.fetchone(
            f"SELECT * from players WHERE discordid = '{str(ctx.message.author).lower()}'"
        )
        if int(pplayer[14]) == 1:
    ...
    async def _topplayed(ctx):
        ...
        for each in lsplayer:
            ...
            msg = (
                msg
                + f"#{nom} **{lsplay[1].title()}** from **{lsplay[15].capitalize()}** total play time **{lspago}**\n"
            )
        embed = discord.Embed(
            title="Top 10 highest play time in the cluster:",
            description=msg,
            color=INFO_COLOR,
        )
        await messagesend(ctx, embed, allowgeneral=False, reject=True)
    ...
    async def _newest(ctx):
        try:
            newlist = await asyncgetnewestplayers(
                "all", fmt="list", last=5, steamid=True
            )
            ...
            for each in newlist:
                ...
                lspago = elapsedTime(
                    Now(), lsplayer["lastseen"], nowifmin=False, append="ago"
                )
                msg = (
                    msg
                    + f'**{lsplayer["playername"].title()}** joined ***{lsplayer["server"].capitalize()}***  -  {lspago}\n'
                )
    ...
    async def _myinfo(ctx):
        try:
            ...
            log.debug(
                f"myinfo request from {whofor} passed, showing info for player {kuser[1]}"
            )
            ...
            xfertable = await db.fetchall(
                f"""SELECT * FROM transferpoints WHERE steamid = '{kuser[0]}'"""
            )
            ...
            msg = (
                msg
                + f"Your current reward points: **{xferpoints}** (on your home server)\n"
            )
            msg = (
                msg
                + f"Your home server is: **{kuser[15].capitalize()}**\nYour total play time is: **{ptime}**\n"
            )
            msg = (
                msg
                + f"You have **{kuser[10]}** current auctions: **{kuser[11]}** Items & **{kuser[12]}** Dinos\n"
            )
            tpwins, twpoints = await asyncgetlottowinnings(kuser[1])
            msg = (
                msg
                + f"Total Lotterys Won: **{tpwins}**  Total Lottery Points Won: **{twpoints}** Points\n"
            )
            woodtime = 1296000
            stonetime = 1987200
            ...
            msg = msg + f"{woodt}\n{stonet}\n{metalt}"
            embed = discord.Embed(
                title=f"Player information for **{kuser[1].capitalize()}**:",
                description=msg,
                color=SUCCESS_COLOR,
            )
        except:
    ...
    async def _winners(ctx):
        last5 = await db.fetchall(
            "SELECT * FROM lotteryinfo WHERE completed = True AND winner != 'None' ORDER BY id DESC LIMIT 5"
        )
        top5 = await db.fetchall(
            "SELECT * FROM players ORDER BY lotterywinnings DESC, lottowins DESC LIMIT 5"
        )
        ...
        try:
            for peach in last5:
                msg = (
                    msg
                    + f'**{peach[6].capitalize()}** won **{peach[1]}** Points  -  {elapsedTime(datetimeto(peach[2] + timedelta(hours=int(peach[5])), fmt="epoch"),Now())} ago\n'
                )
            embed = discord.Embed(title=msg2, description=msg, color=INFO_COLOR)
            await messagesend(ctx, embed, allowgeneral=False, reject=True)
            ...
            for heach in newtop:
                ...
                msg = (
                    msg
                    + f"#{ccount} **{heach[1].capitalize()}** with **{heach[18]}** Lottery Wins.  **{heach[19]}** Total points won.\n"
                )
        except:
    ...
    async def _decay(ctx):
        ...
        msg = (
            msg
            + f"Thatch: **9.5 Days**, Greenhouse: **9.5 Days** (Use MetalGlass for **40 Day** Greenhouse).\n\n"
        )
        if kuser:
            if kuser[8] != whofor:
                ...
                msg = (
                    msg
                    + f"Your discord account is not linked, I cannot determine your decay time left."
                )
            else:
                log.debug(
                    f"decay request from {whofor} accepted, showing detailed info"
                )
                msg = (
                    msg
                    + f"Assuming you were in render range and no other tribe members on, decay time left since last online for **{kuser[1].capitalize()}**:\n"
                )
                woodtime = 1310400
                stonetime = 1969200
    ...
    async def _event(ctx):
        ...
        try:
            ...
            if nextevent:
                embed.add_field(
                    name=f"Next event is {nextevent[4]}",
                    value=f'{nextevent[5]}\nNext event starts in {elapsedTime(datetimeto(d2dt_maint(nextevent[2]), fmt="epoch"), Now())}',
                    inline=False,
                )
            else:
                embed.add_field(
                    name=f"Next Event is not scheduled yet",
                    value=f"Check back again soon for the next event",
                    inline=False,
                )
    ...
    async def _lastrestart(ctx, *args):
        if args:
            ...
            for each in await asyncgetinstancelist():
                ...
                msg = (
                    msg
                    + f"**{each.capitalize()}** last restarted **{lastwipet} ago** for a {await asyncgetlastrestartreason(each)}\n"
                )
            embed = discord.Embed(description=msg, color=INFO_COLOR)
            await messagesend(ctx, embed, allowgeneral=False, reject=True)
    ...
    async def _lastwipe(ctx, *args):
        if args:
            ...
            for each in await asyncgetinstancelist():
                ...
                msg = (
                    msg
                    + f"Last wild dino wipe for **{each.capitalize()}** was **{lastwipet} ago**\n"
                )
            embed = discord.Embed(description=msg, color=INFO_COLOR)
            await messagesend(ctx, embed, allowgeneral=False, reject=True)
    ...
    async def _timeleft(ctx, *args):
        if args:
            instr = args[0].lower()
            if instr in await asyncgetinstancelist():
                restdata = await db.fetchone(
                    f"SELECT needsrestart, restartcountdown, restartreason FROM instances where name = '{instr}'"
                )
            ...
            msg = ""
            restdata = await db.fetchall(
                f"SELECT name, needsrestart, restartcountdown, restartreason FROM instances"
            )
            for instdata in iter(restdata):
                if instdata["needsrestart"] == "False":
                    msg = (
                        msg
                        + f'Server **{instdata["name"].title()}** is not currently in a restart countdown\n'
                    )
                else:
                    msg = (
                        msg
                        + f'Server **{instdata["name"].title()}** has **{instdata["restartcountdown"]}** minutes left until restarting for {instdata["restartreason"]}\n'
                    )
            embed = discord.Embed(description=msg, color=INFO_COLOR)
            await messagesend(ctx, embed, allowgeneral=True, reject=True)
    ...
    async def _who(ctx):
        ...
        for each in await asyncgetinstancelist():
            ...
            tcnt = tcnt + pcnt
        embed = discord.Embed(
            title=f" **{tcnt}**  total players currently online in the cluster",
            color=INFO_COLOR,
        )
        for each in await asyncgetinstancelist():
            pcnt = await asyncgetplayersonline(each, fmt="count")
            ...
            if pcnt != 0:
                embed.add_field(
                    name=f"{each.capitalize().strip()} has  **{pcnt}**  players online:",
                    value=f"{plist}",
                    inline=False,
                )
            else:
                embed.add_field(
                    name=f"{each.capitalize().strip()} has no players online",
                    value="\u200b",
                    inline=False,
                )
        await messagesend(ctx, embed, allowgeneral=True, reject=False)
    ...
    async def _test(ctx):
        try:
            ...
            await asyncwriteglobal("coliseum", "LOTTERY", bcast)
    ...
    async def _today(ctx):
        ...
        for each in await asyncgetinstancelist():
            ...
            tcnt = tcnt + pcnt
        embed = discord.Embed(
            title=f" **{tcnt}**  total players online in the last 24 hours",
            color=INFO_COLOR,
        )
        for each in await asyncgetinstancelist():
            pcnt = await getplayerstoday(each, fmt="count")
            ...
            if pcnt != 0:
                embed.add_field(
                    name=f"{each.capitalize()} has had  **{pcnt}**  players today:",
                    value=f"{plist}",
                    inline=False,
                )
            else:
                embed.add_field(
                    name=f"{each.capitalize()} has had no players today",
                    value="\u200b",
                    inline=False,
                )
        await messagesend(ctx, embed, allowgeneral=True, reject=False)
    ...
    async def _kickme(ctx):
        ...
        if Now() - float(kuser[2]) > 300:
            ...
            log.debug(
                f"kickme request from {whofor} passed, kicking player on {kuser[3]}"
            )
            msg = f"Kicking **{kuser[1].capitalize()}** from the ***{kuser[3].capitalize()}*** server"
            await db.update(
                f"INSERT INTO kicklist (instance,steamid) VALUES ('{kuser[3]}','{kuser[0]}')"
            )
            embed = discord.Embed(description=msg, color=SUCCESS_COLOR)
    ...
    async def _linkme(ctx, *args):
        whofor = str(ctx.message.author).lower()
        dplayer = await db.fetchone(
            f"SELECT playername FROM players WHERE discordid = '{whofor}'"
        )
        if dplayer:
            log.info(
                f"link account request on discord from {whofor} denied, already linked"
            )
            msg = f"Your discord account is already linked to your in-game player **{dplayer[0].title()}**"
            embed = discord.Embed(description=msg, color=FAIL_COLOR)
        else:
            if args:
                reqs = await db.fetchone(
                    f"SELECT * FROM linkrequests WHERE reqcode = '{args[0]}'"
                )
                if reqs:
                    log.success(
                        f"Link on discord from [{whofor.title()}] successful for player [{reqs[1].title()}]"
                    )
                    await db.update(
                        f"UPDATE players SET discordid = '{whofor}' WHERE steamid = '{reqs[0]}'"
                    )
                    await db.update(
                        f"DELETE FROM linkrequests WHERE reqcode = '{args[0]}'"
                    )
                    ...
                    log.warning(
                        f"link account request on discord from {whofor} denied, code not found"
                    )
                    msg = f"That link request code was not found. You must get a link code from typing !linkme in-game"
                    embed = discord.Embed(description=msg, color=FAIL_COLOR)
            else:
                log.warning(
                    f"link account request on discord from {whofor} denied, no code specified"
                )
    ...
    async def _lastseen(ctx, *, arg):
        ...
        if not flast:
            ...
            if plasttime != "now":
                ...
                msg = (
                    f"**{seenname.title()}** is online now on ***{srv.capitalize()}***"
                )
                embed = discord.Embed(description=msg, color=SUCCESS_COLOR)
                await messagesend(ctx, embed, allowgeneral=False, reject=True)
    ...
    async def _myhome(ctx, *args):
        ...
        if kuser:
            if args:
                ...
                log.log(
                    "PLAYER", f"Home server request granted for [{kuser[1].title()}]"
                )
            ...
            log.warning(
                f"Home server request from [{whofor.title()}] denied, no account linked"
            )
    ...
    async def _lotto(ctx, *args):
        ...
        if args:
            if args[0].lower() == "enter" or args[0].lower() == "join":
                lpinfo = await db.fetchone(
                    f"SELECT * FROM players WHERE discordid = '{whofor}'"
                )
                if not lpinfo:
                    log.warning(
                        f"Lottery join request from [{whofor.title()}] denied, account not linked"
                    )
                    ...
                    whofor = lpinfo[1]
                    lpcheck = await db.fetchone(
                        f"SELECT * FROM lotteryplayers WHERE steamid = '{lpinfo[0]}'"
                    )
                    lfo = "Reward Points"
                    # ltime = epochto(float(linfo[3]) + (Secs['hour'] * int(linfo[5])), 'string', est=True)
                    if lpcheck is None:
                        await db.update(
                            "INSERT INTO lotteryplayers (steamid, playername, timestamp, paid) VALUES ('%s', '%s', '%s', '%s')"
                            % (lpinfo[0], lpinfo[1], Now(fmt="dt"), 0)
                        )
                        await db.update(
                            "UPDATE lotteryinfo SET payout = '%s', players = '%s' WHERE id = %s"
                            % (
                                linfo["payout"] + linfo["buyin"] * 2,
                                linfo["players"] + 1,
                                linfo["id"],
                            )
                        )
                        ...
                        log.log(
                            "LOTTO",
                            f"Player [{whofor.title()}] has joined the current active lottery",
                        )
                        if Now(fmt="dt") - await getlastannounce(
                            "lastlottoannounce"
                        ) > timedelta(hours=1):
                            embed2 = discord.Embed(
                                title=f"A player has entered the lottery!",
                                color=INFO_COLOR,
                            )
                            embed2.set_author(
                                name="Galaxy Cluster Reward Point Lottery",
                            ...
                            embed2.add_field(
                                name=f"Current lottery has risen to **{linfo['payout'] + linfo['buyin'] * 2} Points**",
                                value=f"**{linfo['players'] + 1}** Players have entered into this lottery so far\nLottery ends in **{elapsedTime(datetimeto(linfo['startdate'] + timedelta(hours=linfo['days']), fmt='epoch'),Now())}**\n**`!lotto enter`** to join the lottery\n**`!points`** for more information\n**`!winners`** for recent results",
                                inline=True,
                            )
            ...
            if linfo:
                ...
                embed = discord.Embed(
                    title=f"Current lottery is up to {linfo['payout']} reward points",
                    description=f"{linfo['players']} players have entered into this lottery so far",
                    color=SUCCESS_COLOR,
                )
                embed.add_field(
                    name=f'Lottery ends in {elapsedTime(datetimeto(linfo["startdate"] + timedelta(hours=linfo["days"]), fmt="epoch"),Now())}',
                    value=msg,
                    inline=False,
                )
                if str(ctx.message.channel) == "bot-channel":
    ...
    async def on_message(message):
        ...
        if str(message.author) == "Galaxy Cluster#7499":
            ...
            message.content.lower().find("join the server") != -1
            or message.content.lower().find("how do i join") != -1
            or message.content.lower().find("server link") != -1
            or message.content.lower().find("mod collection") != -1
            or message.content.lower().find("mod list") != -1
            or message.content.lower().find("link to server") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [join server] chat for [{message.author}] on [{message.channel}]",
            )
            ...
            message.content.lower().find("what are the rates") != -1
            or message.content.lower().find("server rates") != -1
            or message.content.lower().find("tame rate") != -1
            or message.content.lower().find("harvest rate") != -1
            or message.content.lower().find("current rates") != -1
            or message.content.lower().find("breeding rate") != -1
            or message.content.lower().find("rates here") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [server rates] chat for [{message.author}] on [{message.channel}]",
            )
            ...
            message.content.lower().find("servers up") != -1
            or message.content.lower().find("servers down") != -1
            or message.content.lower().find("server up") != -1
            or message.content.lower().find("server down") != -1
            or message.content.lower().find("server status") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [server up/down] chat for [{message.author}] on [{message.channel}]",
            )
            ...
            message.content.lower().find("isn't that right bot") != -1
            or message.content.lower().find("isnt that right bot") != -1
            or message.content.lower().find("right bot?") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [right bot?] chat from [{message.author}] on [{message.channel}]",
            )
            msg = "I dont know shit."
            await message.channel.send(msg)

        elif (
            message.content.lower().find("hi bot") != -1
            or message.content.lower().find("hello bot") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [hello bot] chat from [{message.author}] on [{message.channel}]",
            )
            msg = "Hello"
            await message.channel.send(msg)

        elif (
            message.content.lower().find("wb bot") != -1
            or message.content.lower().find("welcome back bot") != -1
        ):
            log.log(
                "CMD",
                f"Responding to [wb bot] chat from [{message.author}] on [{message.channel}]",
            )
            msg = "It's great to be back"
            await message.channel.send(msg)

        elif str(message.channel) == "server-chat":
            whos = await db.fetchone(
                "fSELECT playername FROM players WHERE discordid = '{str(message.author).lower()}'"
            )
            if whos:
                await asyncwriteglobal(
                    "discord", whos[0], str(message.content).strip("'")
                )
            ...
            if (
                ...
                log.log(
                    "CRASH",
                    f'{inst.upper()} Server has crashed at {Now(fmt="string", est=True)} EST',
                )
                await db.update(
                    "UPDATE instances SET isup = 0, lastcrash = '%s', lastrestart = '%s' where name = '%s'"
                    % (Now(fmt="dt"), str(Now(fmt="epoch")), inst)
                )
