...
async def asynccheckifenabled(inst):
    instdata = await db.fetchone(f"SELECT enabled, isrunning FROM instances WHERE name = '{inst}'")
    if serverneedsrestart():
        await db.update(f"UPDATE instances SET restartserver = True WHERE name = '{inst.lower()}'")
    if instdata['enabled'] and instdata['isrunning'] == 0 and f'{inst}-restart' not in globvars.taskworkers:
        log.log('MAINT', f'Instance [{inst.title()}] is set to [enabled]. Starting server')
        restartinstnow(inst, startonly=True)
    elif not instdata['enabled'] and instdata['isrunning'] == 1 and f'{inst}-restart' not in globvars.taskworkers:
...
@log.catch
def asyncarkupdater(stop_event):
