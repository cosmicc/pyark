...
async def asyncupdatetimer(inst, ctime):
    await db.update(
        f"UPDATE instances SET restartcountdown = '{ctime}' WHERE name = '{inst}'"
    )
...
async def asyncsetpendingcfgver(inst, cver):
    await db.update(
        f"UPDATE instances SET pendingcfg = {int(cver)} WHERE name = '{inst}'"
    )
...
async def asyncgetlastmaint(svr):
    lastmaint = await db.fetchone(
        f"SELECT * FROM lastmaintenance WHERE name = '{svr.upper()}'"
    )
...
async def asyncsetlastmaint(svr):
    await db.update(
        f"UPDATE lastmaintenance SET lastmaint = '{Now(fmt='dtd')}' WHERE name = '{svr.upper()}'"
    )
...
async def asyncrestartloop(inst, startonly=False):
    ...
    if not await instancestate.check(
        ...
        if not await instancestate.check(
            ...
            if (
                await instancevar.getint(inst, "playersconnected") == 0
            ...
            if (
                ...
                await instancestate.set(inst, "restartwaiting")
                while (
                    await asyncstillneedsrestart(inst)
                    and await instancevar.getint(inst, "playersactive") != 0
                    and timeleft != 0
                    and await instancevar.getint(inst, "playersonline") != 0
                ):
...
async def asynccheckmaint(instances):
    ...
    for inst in instances:
        if (
            inmaint
            and await asyncgetlastmaint(hstname) < Now(fmt="dtd")
            and not await instancestate.check(inst, "maintenance")
        ):
            log.log(
                "MAINT",
                f"Daily maintenance window has opened for server [{hstname.upper()}]...",
            )
            maint = True
    if maint:
        for inst in instances:
        ...
        for inst in instances:
            if await instancevar.getbool(inst, "islistening"):
                ...
                if serverneedsrestart():
                    await db.update(
                        f"UPDATE instances SET restartserver = True WHERE name = '{inst.lower()}'"
                    )
                try:
                    log.log(
                        "MAINT", f"Performing a world data save on [{inst.title()}]..."
                    )
                    await asyncserverexec(["arkmanager", "saveworld", f"@{inst}"])
                    await asyncio.sleep(30)
                    log.log(
                        "MAINT",
                        f"Backing up server instance and archiving old players [{inst.title()}]...",
                    )
                    ...
                    log.log(
                        "MAINT",
                        f"Running all dino and map maintenance on server [{inst.title()}]...",
                    )
                    await asyncwipeit(
                        inst, dinos=False, eggs=True, dams=True, mating=True, bees=False
                    )
                    await asyncio.sleep(30)
                    ...
                    if eventreboot:
                        ...
                        Now() - int(lstsv) > Secs["3day"]
                        or await asyncgetcfgver(inst)
                        < await asyncgetpendingcfgver(inst)
                        or serverneedsrestart()
                    ):
        ...
        if serverneedsrestart():
            log.warning(
                f"[{hstname.upper()}] server needs a hardware reboot after package updates"
            )
...
async def asyncinstancerestart(inst, reason, startonly=False):
    ...
    if not await instancestate.check(
        inst, "restarting"
    ) or not await instancestate.check(inst, "restartwaiting"):
        await db.update(
            f"UPDATE instances SET restartreason = '{reason}' WHERE name = '{inst}'"
        )
...
async def asyncisnewarkver(inst):
    try:
        isarkupd = await asyncserverexec(
            ["arkmanager", "checkupdate", f"@{inst}"], wait=True
        )
...
async def asynccheckbackup(instances):
    for inst in instances:
        ...
        if (
            (lt > 21600 and lt < 21900)
            or (lt > 43200 and lt < 43500)
            or (lt > 64800 and lt < 65100)
        ):
...
async def asynccheckifenabled(inst):
    ...
    if (
        instdata["enabled"]
        and not await instancevar.getbool(inst, 'isrunning')
        and not await instancestate.check(inst, "restartwaiting")
        and not await instancestate.check(inst, "restarting")
    ):
        log.log(
        ...
        log.warning(
            f"Instance [{inst.title()}] is running and set to [disabled]. Stopping server"
        )
...
async def asynccheckupdates(instances):
    global updgennotify
    if is_arkupdater:
        log.debug(f"running updatecheck for {hstname}")
        try:
            ...
            if not ustate:
                ...
                log.log(
                    "UPDATE",
                    f"ARK update found ({curver}>{avlver}) downloading update.",
                )
                await asyncserverexec(
                    ["arkmanager", "update", "--downloadonly", f"@{instances[0]}"]
                )
                ...
                await asyncwritediscord(
                    "ARK Game Update",
                    Now(),
                    name="https://survivetheark.com/index.php?/forums/forum/5-changelog-patch-notes",
                    server="UPDATE",
                )
                msg = f"Ark Game Updare Released\nhttps://survivetheark.com/index.php?/forums/forum/5-changelog-patch-notes"
                log.log(
                    "UPDATE",
                    f"ARK update download complete. Update is staged. Notifying servers",
                )
                await db.update(
                    f"UPDATE instances set needsrestart = 'True', restartreason = 'ark game update'"
                )
    ...
    for inst in instances:
        checkdirs(inst)
        ismodupdd = await asyncserverexec(
            ["arkmanager", "checkmodupdate", f"@{inst}"], wait=True
        )  # #############
        ...
        for teach in ismodupd:
            if (
                teach.find("has been updated") != -1
                or teach.find("needs to be applied") != -1
            ):
                modchk += 1
                al = teach.split(" ")
                modid = al[1]
        ...
        if modchk != 0:
            ...
            log.log(
                "UPDATE",
                f"ARK mod update [{modname}] id [{modid}] detected for instance [{inst.title()}]",
            )
            log.debug(f"downloading mod updates for instance {inst}")
            await asyncserverexec(
                ["arkmanager", "update", "--downloadonly", "--update-mods", f"@{inst}"]
            )
            log.debug(f"mod updates for instance {inst} download complete")
            aname = f"{modname} Mod Update"
            await asyncwritediscord(
                f"{modname} Mod Update",
                Now(),
                name=f"https://steamcommunity.com/sharedfiles/filedetails/changelog/{modid}",
                server="UPDATE",
            )
