...
async def asyncrespmyinfo(inst, whoasked):
    ...
    if steamid:
        player = await db.fetchone(
            f"SELECT * FROM players WHERE playername = '{whoasked}' ORDER BY lastseen DESC"
        )
    ...
    try:
        ...
        if inst not in await asyncgetinstancelist():
            ...
            players = await db.fetchall(
                f"SELECT * FROM players WHERE server = '{inst}'"
            )
            pcnt = 0
            plist = ""
...
async def asyncpopulatevoters(inst):
    ...
    playercount = 0
    players = await db.fetchall(
        f"SELECT * FROM players WHERE server = '{inst}' and online = True"
    )
    for player in players:
...
async def asyncresetlastvote(inst):
    await db.update(
        f"UPDATE instances SET lastvote = '{int(time())}' WHERE name = '{inst}'"
    )


async def asyncresetlastwipe(inst):
    await db.update(
        f"UPDATE instances SET lastdinowipe = '{int(time())}' WHERE name = '{inst}'"
    )
...
async def processtcdata(inst, tcdata):
    ...
    if not player:
        ...
        if playername.lower() != player["playername"].lower():
            log.log(
                "UPDATE",
                f'Player name update for [{player["playername"]}] to [{playername}]',
            )
            await db.update(
                f"UPDATE players SET playername = '{playername}' WHERE steamid = '{steamid}'"
            )
        if inst == player["homeserver"]:
            log.trace(
                f"player {playername} with steamid {steamid} was found on HOME server {inst}. updating info."
            )
            await db.update(
                f"UPDATE players SET playedtime = '{playtime}', rewardpoints = '{rewardpoints}' WHERE steamid = '{steamid}'"
            )
        else:
            if (
                Now() - int(player["lastseen"]) < 200
                and Now() - player["lastlogin"] > 30
                and player["server"] == inst
            ):
                log.trace(
                    f"player {playername} with steamid {steamid} was found on NON-HOME server {inst}. updating info"
                )
                transferdata = await db.fetchone(
                    f"SELECT * from transferpoints WHERE steamid = '{steamid}' and server = '{inst}'"
                )
                if transferdata:
                    await db.update(
                        f"UPDATE transferpoints SET points = '{rewardpoints}' WHERE steamid = '{steamid}' and server = '{inst}'"
                    )
                elif rewardpoints != 0:
                    await db.update(
                        f"INSERT INTO transferpoints (steamid, server, points) VALUES ('{steamid}', '{inst}', {rewardpoints})"
                    )
...
async def asynchomeserver(inst, whoasked, ext):
    ...
    if steamid:
        ...
        if ext != "":
            ...
            if ext in tservers:
                if ext != player["homeserver"]:
                    if inst == player["homeserver"]:
                        log.log(
                            "PLAYER",
                            f'[{player["playername"].title()}] has transferred home servers from [{player["homeserver"].title()}] to [{ext.title()}] with {player["rewardpoints"]} points',
                        )
                        command = "tcsar setarctotal {steamid} 0"
                        await asyncserverscriptcmd(inst, command)
                        await db.update(
                            f"""UPDATE players SET homemovepoints = {player["rewardpoints"]}, homeserver = '{ext}' WHERE steamid = '{steamid}'"""
                        )
            ...
            message = (
                f'Your current home server is: {player["homeserver"].capitalize()}'
            )
            await asyncserverchatto(inst, steamid, message)
...
async def asynclotteryinfo(lottery, player, inst):
    ...
    await asyncserverchatto(inst, player["steamid"], message)
    inlotto = await db.fetchone(
        f"""SELECT * FROM lotteryplayers WHERE steamid = '{player["steamid"]}'"""
    )
    if inlotto:
...
async def asynclottery(whoasked, lchoice, inst):
    ...
    if lottery:
        if lchoice == "join" or lchoice == "enter":
            log.log(
                "CMD",
                f"Responding to a [!lotto join] request from [{whoasked.title()}] on [{inst.title()}]",
            )
            ...
            if lpcheck is None:
                await db.update(
                    f"""INSERT INTO lotteryplayers (steamid, playername, timestamp, paid) VALUES ('{player["steamid"]}', '{player["playername"]}', '{Now(fmt='dt')}', 0)"""
                )
                await db.update(
                    f"""UPDATE lotteryinfo SET payout = {lottery['payout'] + lottery['buyin'] * 2}, players = {lottery['players'] + 1} WHERE completed = False"""
                )
                ...
                log.log(
                    "LOTTO",
                    f"Player [{whoasked.title()}] has joined the current active lottery",
                )
            else:
            ...
            log.log(
                "CMD",
                f"Responding to a [!lotto] request from [{whoasked.title()}] on [{inst.title()}]",
            )
...
async def playerjoin(line, inst):
    ...
    if player:
        ...
        steamid = player["steamid"]
        await db.update(
            f"""UPDATE players SET online = True, refreshsteam = True, lastlogin = '{Now()}', lastseen = '{Now()}', refreshauctions = True, server = '{inst}', connects = {player["connects"] + 1} WHERE steamid = '{steamid}'"""
        )
...
async def playerleave(line, inst):
    ...
    if player:
        ...
        log.error(
            f"Player with steam name [{newline[1].strip()}] was not found while leaving server"
        )
...
def deconstructchatline(line):
    try:
        ...
        chattime = datetime.strptime(
            line.split(":", 1)[0].strip('"'), "%Y.%m.%d_%H.%M.%S"
        )
...
async def asyncchatlinedetected(inst, chatdict):
    ...
    ttrans = partial(trans_to_eng, chatdict["line"])
    asyncloop = asyncio.get_running_loop()
    transmsg = await asyncloop.run_in_executor(None, ttrans)
    tstamp = chatdict["time"].strftime("%m-%d %I:%M%p")
    log.log("CHAT", f'{inst.upper():>8}| {chatdict["name"]}: {transmsg}')
    await asyncwritechat(
        inst, chatdict["name"].lower(), transmsg.replace("'", ""), tstamp
    )
...
async def cmdsexecute(inst):
    ...
    factory = partial(
        ...
        cmd_done,
        inst,
        parsetask=asyncprocesscmdline,
        finishtask=asyncfinishcmd,
    )
...
async def cmdscheck(instances):
    for inst in instances:
        if await instancevar.getbool(
            inst, "islistening"
        ) and not await instancestate.check(inst, "cmdcheck"):
