...
async def test_openfiles():
    assert type(await modules.servertools.openfiles()) is tuple
    assert await modules.servertools.openfiles() is not None
    assert len(await modules.servertools.openfiles()) == 2
    results = await modules.servertools.openfiles()
