...
# Instance Json log
log.add(sink=str(jsonlogfile), level=19, buffering=1, enqueue=True, backtrace=False, diagnose=False, serialize=True, colorize=True, format=shortlogformat, rotation="5 MB", retention="1 month", compression="tar.gz")
...
if loglevel == 'DEBUG' or loglevel == 'TRACE':
    if loglevel == 'DEBUG':
        ...
        lev = 5
    log.add(sink=str(jsondebugfile), level=lev, buffering=1, enqueue=True, backtrace=True, diagnose=True, serialize=True, colorize=True, format=shortlogformat, delay=False, filter=checkdebuglog, rotation="5 MB", retention="1 month", compression="tar.gz")
