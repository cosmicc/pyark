...
def test_epochto():
    ...
    assert (
        type(timehelper.epochto(datetime.now().timestamp(), "dt", est=True)) is datetime
    )
    assert type(timehelper.epochto(datetime.now().timestamp(), "string")) is str
    assert (
        type(timehelper.epochto(datetime.now().timestamp(), "string", est=True)) is str
    )
...
def test_elapsedTime():
    ...
    assert (
        timehelper.elapsedTime("1541300689", "1541300681", nowifmin=False)
        == "8 seconds"
    )
    assert (
        timehelper.elapsedTime(1541300681.012124, 1541300689.523465, nowifmin=False)
        == "8 seconds"
    )
