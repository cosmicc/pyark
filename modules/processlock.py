...
class PLock:
    ...
    def _aquirelock(self):
        ...
        try:
            ...
            log.exception(
                f"General error trying to lock process to file {self.lockfile}. exiting."
            )
            exit(1)
        else:
            log.debug(
                f"Process [{self.pid}] locked to file [{self.lockfile}] on [{hstname}]"
            )
            return True
    ...
    def _setpidfile(self):
        try:
            ...
            log.debug(
                f"Pid file [{self.pidfile}] holding pid [{self.pid}] for [{hstname}]"
            )
