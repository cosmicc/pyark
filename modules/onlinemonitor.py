...
async def asyncserverisinrestart(steamid, inst, player):
    ...
    if rbt[3] == "True":
        log.log(
            "UPDATE",
            f"Notifying player [{player[1].title()}] that [{inst.title()}] will be restarting in {rbt[7]} min",
        )
...
async def asyncisinlottery(steamid):
    isinlotto = await db.fetchone(
        "SELECT * FROM lotteryinfo WHERE winner = 'Incomplete'"
    )
    if isinlotto:
        isinlotto2 = await db.fetchone(
            f"SELECT * FROM lotteryplayers WHERE steamid = '{steamid}'"
        )
        if isinlotto2:
            return True
        else:
...
async def asynccheckifbanned(steamid):
    oplayer = await db.fetchone(
        f"SELECT steamid FROM players WHERE steamid = '{steamid}' AND banned != ''"
    )
    bplayer = await db.fetchone(
        f"SELECT steamid FROM banlist WHERE steamid = '{steamid}'"
    )
...
async def asynckickcheck(instances):
    for inst in instances:
        if await instancevar.getbool(inst, "islistening"):
            kicked = await db.fetchone(
                f"SELECT * FROM kicklist WHERE instance = '{inst}'"
            )
            if kicked:
                await asyncserverrconcmd(inst, f"kickplayer {kicked[1]}")
                log.log(
                    "KICK",
                    f"Kicking user [{kicked[1]}] from server [{inst.title()}] on kicklist",
                )
...
async def asynconlinedblchecker(instances):
    for inst in instances:
        ...
        players = await db.fetchall(
            f"SELECT * FROM players WHERE online = True AND lastseen <= {Now() - 300} AND server = '{inst}'"
        )
        for player in players:
            log.warning(
                f'Player [{player["playername"].title()}] wasnt seen logging off [{inst.title()}] Clearing player from online status'
            )
            await db.update(
                "UPDATE players SET online = False, welcomeannounce = True, refreshsteam = True, server = '%s' WHERE steamid = '%s'"
                % (player["server"], player["steamid"])
            )
...
async def onlineexecute(inst):
    ...
    factory = partial(
        ...
        cmd_done,
        inst,
        parsetask=asyncprocessonline,
        finishtask=asyncfinishonline,
    )
...
async def onlinecheck(instances):
    for inst in instances:
        if await instancevar.getbool(
            inst, "islistening"
        ) and not await instancestate.check(inst, "playercheck"):
