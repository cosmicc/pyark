...
async def test_asyncgetplayerinfo():
    assert (
        type(await modules.players.asyncgetplayerinfo(steamid="76561198408657294"))
        is Record
    )
    ...
    assert (
        type(await modules.players.asyncgetplayerinfo(discordid="Rykker#7393"))
        is Record
    )
    player = await modules.players.asyncgetplayerinfo(discordid="Rykker#7393")
    assert player["playername"] == "rykker"
    assert (
        type(await modules.players.asyncgetplayerinfo(steamname="Galaxy-Cluster"))
        is Record
    )
...
async def test_asyncgetplayersonline():
    assert (
        type(await modules.players.asyncgetplayersonline("ragnarok", fmt="count"))
        is int
    )
    assert (
        type(await modules.players.asyncgetplayersonline("ragnarok", fmt="string"))
        is str
    )
    assert (
        type(await modules.players.asyncgetplayersonline("ragnarok", fmt="list"))
        is list
    )
...
async def test_asyncgettopplayedplayers():
    assert (
        type(await modules.players.asyncgettopplayedplayers("ragnarok", fmt="list"))
        is list
    )
    assert (
        type(await modules.players.asyncgettopplayedplayers("ragnarok", fmt="string"))
        is str
    )
    assert (
        type(await modules.players.asyncgettopplayedplayers("all", fmt="list")) is list
    )
    assert (
        type(await modules.players.asyncgettopplayedplayers("all", fmt="string")) is str
    )
    assert (
        len(await modules.players.asyncgettopplayedplayers("all", fmt="list", last=10))
        == 10
    )
...
async def test_asyncgetnewestplayers():
    assert (
        type(await modules.players.asyncgetnewestplayers("ragnarok", fmt="list"))
        is list
    )
    assert (
        type(await modules.players.asyncgetnewestplayers("ragnarok", fmt="string"))
        is str
    )
    ...
    assert (
        len(await modules.players.asyncgetnewestplayers("all", fmt="list", last=10))
        == 10
    )
