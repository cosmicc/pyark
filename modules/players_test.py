...
async def test_asyncgetplayerinfo():
    assert await type(modules.players.asyncgetplayerinfo(steamid='76561198408657294')) is Record
    ...
    assert await type(modules.players.asyncgetplayerinfo(playername='Admin')) is Record
    assert await len(modules.players.asyncgetplayerinfo(playername='Admin')) == 1
    assert await type(modules.players.asyncgetplayerinfo(discordid='Rykker#7393')) is Record
    ...
    assert await type(modules.players.asyncgetplayerinfo(steamname='Galaxy-Cluster')) is Record
