...
async def asyncgetplayerlastseen(steamid: Optional[str]=None, playername: Optional[str]=None) -> Optional[int]:
    if playername:
        dbdata = await db.fetchone(f"SELECT lastseen FROM players WHERE playername = '{playername.lower()}'")
    elif steamid:
...
async def asyncgetplayersonline(instance: str, fmt: str='count'):
    if instance.lower() == 'all':
        ...
        dbdata = await db.fetchall(f"SELECT playername FROM players WHERE online = True AND server = '{instance.lower()}' ORDER BY lastseen DESC")
    if fmt == 'count':
        return len(dbdata)
    elif fmt == 'string':
        players = []
        for player in iter(dbdata):
            players.append(dbdata[player]['playername'])
        return ', '.join(players)
    elif fmt == 'list':
        players = []
        for player in iter(dbdata):
