...
from modules.tribes import (
    asyncgettribeinfo,
    asyncputplayerintribe,
    asyncremoveplayerintribe,
)
...
def checkgamelog(record):
    if (
        record["level"] == "TRAP"
        or record["level"] == "ADMIN"
        or record["level"] == "DEATH"
        or record["level"] == "TAME"
        or record["level"] == "DECAY"
        or record["level"] == "DEMO"
        or record["level"] == "TRIBE"
        or record["level"] == "CLAIM"
        or record["level"] == "RELEASE"
    ):
        return True
    else:
        return False
...
async def _processgameline(inst, ptype, line):
    ...
    logheader = (
        f'{Now(fmt="dt").strftime("%Y-%m-%d %H:%M:%S")}|{inst.upper():>8}|{ptype:<7}| '
    )
    ...
    if ptype == "TRAP":
        ...
        line = (
            f"{logheader}[{playername.title()}] of ({tribename}) has trapped [{dino}]"
        )
        clog.log(ptype, line)
    elif ptype == "RELEASE":
        tribename, tribeid = await asyncgettribeinfo(linesplit, inst, ptype)
        ...
        dino = msgsplit[1].strip().replace(")", "").replace("(", "")
        line = (
            f"{logheader}[{playername.title()}] of ({tribename}) has released [{dino}]"
        )
        clog.log(ptype, line)
    elif ptype == "DEATH":
        ...
        if tribename is None:
            ...
            if deathsplit[1].find("was killed by") != -1:
                ...
                playerlevel = (
                    deathsplit[1].split("was killed by")[0].strip().replace("()", "")
                )
        ...
        if tribename is None:
            ...
            await asyncputplayerintribe(tribeid, playername)
            if (
                len(linesplit[2].split(" demolished a ")) > 0
                and linesplit[2].find(" demolished a ") != -1
            ):
                demoitem = (
                    linesplit[2]
                    .split(" demolished a ")[1]
                    .replace("'", "")
                    .strip(")")
                    .strip("!")
                    .strip()
        ...
        if not await asyncisplayeradmin(steamid):
            clog.warning(
                f"{logheader}Admin command [{cmd}] executed by NON-ADMIN [{pname.title()}] !"
            )
            await db.update(
        ...
        if tribeid is not None:
            if linesplit[2].find(" was added to the Tribe by ") != -1:
                playername = (
                    linesplit[2][10:].split(" was added to the Tribe by ")[0].strip()
                )
                playername2 = (
                    linesplit[2][10:]
                    .split(" was added to the Tribe by ")[1]
                    .strip()
                    .strip(")")
                    .strip("!")
                )
                ...
                playername = (
                    linesplit[2][10:].split(" was removed from the Tribe!")[0].strip()
                )
                ...
                playername = (
                    linesplit[2][10:].split(" was added to the Tribe!")[0].strip()
                )
                await asyncputplayerintribe(tribeid, playername)
                ...
                await asyncputplayerintribe(tribeid, playername)
                rankgroup = (
                    linesplit[2][10:].split(" set to Rank Group ")[1].strip().strip("!")
                )
