        ...
        if adata is not None:
            ...
            for auction in iter(adata['Auctions']):
                if auction['Type'] == 'Item':
                    await db.update(f"""INSTERT INTO auctiondata (date, playername, playersteamid, auctiontype, askingclass, askingamount, auctionname, quanity, blueprint) VALUES (auction["Date"], auction["Seller"]["Name"], auction["Seller"]["SteamID"], 'Item', auction["AskingClass"], auction["AskingAmount"], auction["Name"], auction["Quanity"], auction["Items"]["Flags"]["IsBlueprint"])""")
...
async def asyncfetchauctiondata(session, steamid, playername):
    try:
        ...
        log.debug(
            f"Updated Auction API player information for [{playername}] ({steamid})"
        )
        if auctions:
            return auctions
        else:
...
async def asyncwriteauctionstats(steamid, numauctions, numitems, numdinos):
    await db.update(
        "UPDATE players SET totalauctions = '%s', itemauctions = '%s', dinoauctions = '%s' WHERE steamid = '%s'"
...
async def getsteaminfo(steamid, playername, session):
    try:
        ...
        if "lastlogoff" in player:
            ...
            lastlogoff = 0
        await db.update(
            f"""UPDATE players SET steamname = '{player["personaname"]}', steamrealname = '{realname}', steamlastlogoff = {lastlogoff}, steamcreated = {steamcreated}, steamcountry = '{steamcountry}' WHERE steamid = '{steamid}'"""
        )
    except:
        log.exception(
            f"HTTP Error fetching steam api player data for [{playername}] ({steamid})"
        )
        return False
    else:
        log.debug(
            f"Updated steam API player information for [{playername}] ({steamid})"
        )
        return True
...
async def getsteambans(steamid, playername, session):
    try:
        ...
        if player["EconomyBan"] == "none":
            ...
            economyban = True
        await db.update(
            f"""UPDATE players SET steamcommunityban = {player["CommunityBanned"]}, steamvacban = {player["VACBanned"]}, steamvacbannum = {player["NumberOfVACBans"]}, steamgamesbannum = {player["NumberOfGameBans"]}, steamlastbandays = {player["DaysSinceLastBan"]}, steameconomyban = {economyban} WHERE steamid = '{steamid}'"""
        )
    except:
        log.exception(
            f"Error fetching steam api ban data for [{playername}] ({steamid})"
        )
        return False
    else:
...
async def asyncsteamapifetcher(session):
    players = await db.fetchall(
        f"SELECT steamid, playername, steamrefreshtime FROM players WHERE refreshsteam = True"
    )
    if players:
        log.trace(f"Found {len(players)} players for steamapi to process {players}")
        for player in players:
            log.trace(
                f'processing player steamapi [{player["playername"]}] ({player["steamid"]})'
            )
            refresh = False
            if player["steamrefreshtime"]:
                if player["steamrefreshtime"] < Now(fmt="dt") - timedelta(days=1):
                    log.trace(
                        f'player [{player["playername"]}] is past steam refresh time'
                    )
                    refresh = True
                    rtime = Now(fmt="dt")
                else:
                ...
                log.trace(
                    f'no steamrefreshtime found for player [{player["playername"]}]'
                )
                refresh = True
                rtime = Now(fmt="dt")
            await db.update(
                f"UPDATE players SET refreshsteam = False, steamrefreshtime = '{rtime}' WHERE steamid = '{player[0]}'"
            )
            log.trace(
                f'retrieving steam information for player [{player["playername"]}] ({player["steamid"]}]'
            )
            if refresh:
...
async def asyncauctionapifetcher(session):
    players = await db.fetchall(
        f"SELECT steamid, playername, auctionrefreshtime FROM players WHERE refreshauctions = True OR online = True"
    )
    if players:
        log.trace(f"Found {len(players)} players for auctionapi to process {players}")
        for player in players:
            log.trace(
                f"processing player auctionapi [{player['playername']}] ({player['steamid']})"
            )
            refresh = False
            if player["auctionrefreshtime"]:
                if player["auctionrefreshtime"] < Now(fmt="dt") - timedelta(hours=1):
                    log.trace(
                        f"player [{player['playername']}] is past auction refresh time"
                    )
                    refresh = True
                    rtime = Now(fmt="dt")
                else:
                ...
                log.trace(
                    f"no auctionrefreshtime found for player [{player['playername']}]"
                )
                refresh = True
                rtime = Now(fmt="dt")
            await db.update(
                f"UPDATE players SET refreshauctions = False, auctionrefreshtime = '{rtime}' WHERE steamid = '{player[0]}'"
            )
            if refresh:
                log.debug(
                    f"retrieving auction information for player [{player['playername']}] ({player['steamid']}]"
                )
                pauctions = await asyncfetchauctiondata(
                    session, player["steamid"], player["playername"]
                )
                totauctions, iauctions, dauctions = getauctionstats(pauctions)
                await asyncwriteauctionstats(
                    player["steamid"], totauctions, iauctions, dauctions
                )
                log.debug(
                    f"retrieved auctions for player [{player['playername']}] total: {totauctions}, items: {iauctions}, dinos: {dauctions}"
                )
            await asyncio.sleep(5)
