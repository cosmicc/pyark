...
from lottery import totallotterydeposits, isinlottery
...
def discordbot():
    ...
    async def on_message(message):
        ...
        async def discordsend(message, embed, allowgeneral=False, reject=True, adminonly=False):
            try:
                if message.channel.is_private:
                    await client.send_message(message.author, embed=embed)
                elif str(message.channel) != 'bot-channel' or (not allowgeneral and str(message.channel) == 'general-chat'):
                    role = str(discord.utils.get(message.author.roles, name="Cluster Admin"))
                    ...
                    if reject and role != 'Cluster Admin':
                        ...
                        await client.send_message(message.author, embed=rejectembed)
...
{reqs[1]} {whofor} {reqs[0]}')
            ...
            await discordsend(message, embed, allowgeneral=True, reject=False)
            ...
            try:
                ...
                embed.add_field(name=f"Galaxy Cluster Server Rates:", value=f"{eventr}", inline=True)
                await discordsend(message, embed, allowgeneral=True, reject=True)
            except:
        ...
        elif message.content.lower().startswith('!recent') or message.content.lower().startswith('!whorecent') or message.content.lower().startswith('!lasthour'):
            ...
            if len(newname) > 1:
                ...
                if not flast:
                    ...
                    embed = discord.Embed(description=msg, color=FAIL_COLOR)
                    await discordsend(message, embed, allowgeneral=False, reject=True)
                else:
            ...
            if kuser:
                if len(newsrv) > 1:
                    ...
                    embed = discord.Embed(description=msg, color=FAIL_COLOR)
                    await discordsend(message, embed, allowgeneral=False, reject=False)
                else:
