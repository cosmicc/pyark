...
async def _instancestats():
    async def instancestats():
        return await webapp.db.fetchall(
            f"SELECT name, lastdinowipe, lastrestart, lastvote, restartreason, battlemetricslink, arkversion, rank, score, votes, serverhost, connectingplayers, activeplayers, isup, arkbuild FROM instances ORDER BY name DESC"
        )
