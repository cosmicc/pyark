...
async def _last7lotterys():
    async def last7lotterys():
        ...
        for lottery in iter(lotterys):
            num = num + 1
            resp.append({'num': num, 'playername': lottery['winner'], 'points': lottery['payout']})
        return resp
...
async def _otherplayercounts():
    async def otherplayercounts():
        ...
        mcount = await webapp.db.fetchone(f"""SELECT COUNT(*) FROM players WHERE banned = '' AND lastseen >= '{Now() - Secs["month"]}'""")
