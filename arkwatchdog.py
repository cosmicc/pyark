...
def redislistener():
    ...
    for response in pubsub.listen():
        if (
            ...
            and response["channel"].decode() == f"{hstname.lower()}-commands"
        ):
            ...
            if response["data"].decode() == "restart":
                ...
                subprocess.run(
                    ["systemctl", "stop", "pyark"], shell=False, capture_output=False
                )
            elif response["data"].decode() == "start":
                subprocess.run(
                    ["systemctl", "start", "pyark"], shell=False, capture_output=False
                )
                ...
                if config.get("general", "loglevel") != loglevel.upper():
                    log.debug(
                        f"Changing pyark loglevel from [{config.get('general', 'loglevel')}] to [{loglevel.upper()}] on [{hstname}]"
                    )
                ...
                if len(respsplit) == 4:
                    ...
                    if config.has_section(section):
                        if not config.has_option(section, key):
                            log.info(
                                f"Adding pyark config entry [{key}] in [{section}] from [{config.get(section, key)}] to [{value}] on [{hstname}]"
                            )
                            ...
                            log.info(
                                f"Changing pyark config entry [{key}] in [{section}] from [{config.get(section, key)}] to [{value}] on [{hstname}]"
                            )
                            config.set(section, key, value)
                            with open(str(pyarkcfgfile), "w") as configfile:
                                config.write(configfile)
                            ...
                            log.info(
                                f"Pyark config entry [{key}] in [{section}] already set to [{value}] on [{hstname}]"
                            )
                    else:
                        log.warning(
                            f"Section [{section}] does not exist to add/change option [{key}] on [{hstname}]"
                        )
                ...
                if len(respsplit) == 3:
                    ...
                    if config.has_section(section) and config.has_option(section, key):
                        if config.get(section, key) is None:
                            log.warning(
                                f"Option [{key}] in [{section}] does not exist to remove on [{hstname}]"
                            )
                        elif config.get(section, key):
                            log.info(
                                f"Removing pyark config entry [{key}] in [{section}] value [{config.get(section, key)}] on [{hstname}]"
                            )
                            config.set(section, key, value)
                            with open(str(pyarkcfgfile), "w") as configfile:
                                config.write(configfile)
                        ...
                        log.warning(
                            f"Option [{key}] or [{section}] does not exist to remove on [{hstname}]"
                        )


thread = threading.Thread(
    name="redislistener", target=redislistener, args=(), daemon=True
)
    ...
    try:
        if not pyarkpidfile.is_file() or not pyarklockfile.is_file():
            ...
            if pyarkpid == "" or pyarkpid is None:
                if count == 1:
                    log.error(
                        f"Pyark process is not running. (Empty pid found in pidfile)"
                    )
                count += 1
            else:
