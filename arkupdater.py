...
def instancerestart(inst, reason):
    ...
    if playercount(inst) == 0:
        ...
        subprocess.run('arkmanager --saveworld stop @%s' % (inst),stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
        ...
        log.debug(f'instance {inst} updated config files')
