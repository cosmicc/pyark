...
from modules.timehelper import (
    Now,
    Secs,
    datetimeto,
    elapsedTime,
...
class MessageForm(FlaskForm):
    message = StringField(
        "message", validators=[InputRequired(), Length(min=1, max=30)]
    )
...
def connect():
    socketio.start_background_task(
        target=ChatThread, sid=request.sid, namespace="/debugstream"
    )
    socketio.start_background_task(
        target=GameThread, sid=request.sid, namespace="/debugstream"
    )
    socketio.start_background_task(
        target=DebugLogThread, sid=request.sid, namespace="/debugstream"
    )
...
def connect2():
    socketio.start_background_task(
        target=ChatThread, sid=request.sid, namespace="/logstream"
    )
    socketio.start_background_task(
        target=GameThread, sid=request.sid, namespace="/logstream"
    )
    socketio.start_background_task(
        target=LogThread, sid=request.sid, namespace="/logstream"
    )
    logthreads.append(request.sid)
    log.debug(f"Logstream started for: {request.sid}")
...
def database_processor3():
    def ui_getplayerserver(steamid):
        return dbquery(
            "SELECT server FROM players WHERE steamid = '%s'" % (steamid,),
            fmt="string",
            fetch="one",
        )
...
def _getlotteryplayers():
    def ui_getlotteryplayers(fmt="list"):
        return dbquery(
            "SELECT playername FROM lotteryplayers", fmt=fmt, fetch="all", single=True
        )
...
def database_processor5():
    def ui_getinstver(inst):
        return dbquery(
            "SELECT arkversion FROM instances WHERE name = '%s'" % (inst.lower(),),
            fmt="string",
            fetch="one",
        )
...
def database_processor66():
    def ui_getinstap(inst):
        return dbquery(
            "SELECT activeplayers FROM instances WHERE name = '%s'" % (inst.lower(),),
            fmt="string",
            fetch="one",
        )
...
def _lastactive():
    def ui_lastactive(inst):
        retime = dbquery(
            f"SELECT date FROM {inst}_stats WHERE value != 0 ORDER BY date DESC LIMIT 1",
            fetch="one",
        )[0]
    ...
    def ui_last24avg(inst, dtype):
        ...
        if inst == "all":
            ...
            for each in instancelist():
                ...
                c = conn.cursor()
                c.execute(
                    "SELECT * FROM {}_stats WHERE date > '{}' ORDER BY date DESC".format(
                        each, datetime.now() - timedelta(hours=hours)
                    )
                )
                alllist = c.fetchall()
                for y in alllist:
                ...
                if statavglist == []:
                    ...
                    navglist = [
                        sum(pair)
                        for pair in zip_longest(statlist, statavglist, fillvalue=0)
                    ]
            ...
            df = pd.read_sql(
                f"SELECT * FROM {inst}_stats WHERE date > '{datetime.now() - timedelta(hours=hours)}' ORDER BY date DESC",
                conn,
                parse_dates=["date"],
                index_col="date",
            )
...
def instanceinfo(inst):
    return dbquery(
        "SELECT * FROM instances WHERE name = '%s'" % (inst.lower(),),
        fmt="dict",
        fetch="one",
    )
...
def getlastevent():
    return dbquery(
        "SELECT * FROM events WHERE completed = 1 ORDER BY endtime DESC",
        fmt="dict",
        fetch="one",
    )
...
def getlastlottery():
    return dbquery(
        "SELECT * FROM lotteryinfo WHERE completed = True ORDER BY startdate DESC",
        fmt="dict",
        fetch="one",
    )
...
def getcurrentlottery():
    return dbquery(
        "SELECT * FROM lotteryinfo WHERE completed = False", fmt="dict", fetch="one"
    )
...
def startthelottery(buyin, length):
    litm = buyin * 20
    dbupdate(
        "INSERT INTO lotteryinfo (payout,startdate,buyin,days,players,winner,announced,completed) VALUES ('%s','%s','%s','%s',0,'Incomplete',False,False)"
        % (litm, Now(fmt="dt"), buyin, length)
    )
...
def dashboard():
    ...
    return render_template(
        "dashboard.html",
        loginname=current_user.email,
        instances=instancelist(),
        activeplayers=len(getactiveplayers(Secs["month"])),
        unarchivedplayers=len(getactiveplayers(Secs["3month"])),
        newplayers=len(getnewplayers(Secs["week"])),
        dailyplayers=len(getactiveplayers(Secs["day"])),
        weeklyplayers=len(getactiveplayers(Secs["week"])),
        hitnruns=len(gethitnruns(Secs["week"])),
        newplayersday=len(getnewplayers(Secs["day"])),
    )
...
def serverinfo(instance):
    return render_template(
        "serverinfo.html",
        serverinfo=instanceinfo(instance),
        lastcrash=getlastcrash(instance),
    )
...
def result():
    if request.method == "POST":
        return render_template(
            "playerinfo.html",
            playerinfo=getplayer(steamid=request.form["player"], fmt="dict"),
        )
...
def result454():
    if request.method == "POST":
        return render_template(
            "tribeinfo.html", tribe=gettribe(request.form["tribeid"])
        )
...
def result2():
    if request.method == "POST":
        return render_template(
            "playerinfo.html",
            playerinfo=getplayer(discordid=request.form["player"].lower(), fmt="dict"),
        )
...
def startlottery():
    ...
    if form.validate_on_submit():
        ...
        flash("New Lottery has been Started", "info")
...
def playerinfo(steamid):
    if request.method == "POST":
        serverchat(
            ...
            whosent=steamid,
            inst=getplayer(steamid=steamid, fmt="dict")["server"],
            private=True,
        )
    ...
    return render_template(
        "playerinfo.html",
        playerinfo=getplayer(steamid=steamid, fmt="dict"),
        tribes=getplayertribes(steamid),
    )
...
def deletemessage(messageid):
    if request.method == "POST":
        miq = dbquery(
            "SELECT * FROM messages WHERE id = '%s'" % (messageid,),
            fmt="dict",
            fetch="one",
        )
...
def messages():
    if request.method == "POST":
        if (
            request.form["player"]
            == getplayer(steamid=current_user.steamid, fmt="dict")["playername"]
        ):
            ...
            flash(
                f"You cannot send more then 5 messages that haven't been read yet",
                "error",
            )
        else:
            sendmessage(
                current_user.steamid,
                getplayer(playername=request.form["player"], fmt="dict")["steamid"],
                request.form["message"],
            )
...
def webcreate(steamid):
    ...
    return render_template(
        "webcreate.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
    )
...
def changepass(steamid):
    if request.method == "POST":
        if current_user.has_role("admin") or current_user.steamid == steamid:
            if request.form["password"] == "" or request.form["password_confirm"] == "":
                ...
                dbupdate(
                    "UPDATE web_users SET password = '%s' WHERE steamid = '%s'"
                    % (hash_password(request.form["password"]), steamid)
                )
    ...
    if current_user.has_role("admin") or current_user.steamid == steamid:
        return render_template(
            "changepass.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
        )
...
def webinfo(steamid):
    if request.method == "POST":
        if request.form["btype"] == "Update Settings":
            User.query.filter_by(steamid=steamid).update(
                dict(timezone=request.form["timezone"])
            )
            db.session.commit()
    ...
    if not hasattr(webuser, "email"):
        ...
        return render_template(
            "playerinfo.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
        )
    else:
        if current_user.has_role("admin") or current_user.steamid == steamid:
            return render_template(
...
def _players():
    return render_template(
        ...
        newplayers=getnewplayers(Secs["week"]),
        discordplayers=getdiscordplayers(),
        steamplayers=getsteamnameplayers(),
        tribes=gettribes(),
    )
...
def _bantoggle(steamid):
    if isplayerbanned(steamid=steamid):
        ...
        flash("Player has been Un-Banned", "success")
        return render_template(
            "playerinfo.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
        )
    else:
        banunbanplayer(steamid=steamid, ban=True)
        flash("Player has been BANNED!", "error")
        return render_template(
            "playerinfo.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
        )
...
def _kickplayer(steamid, instance):
    ...
    flash("Kicking player from %s" % (instance,), "info")
    return render_template(
        "playerinfo.html", playerinfo=getplayer(steamid=steamid, fmt="dict")
    )
...
def _stopinstance(instance):
    ...
    flash("Sending Server Shutdown", "info")
    return render_template("serverinfo.html", serverinfo=instanceinfo(instance))
...
def _cancelrestartinstance(instance):
    ...
    flash("Cancelling Server Restart", "info")
    return render_template("serverinfo.html", serverinfo=instanceinfo(instance))
...
def _chatlog(instance):
    ...
    return render_template(
        "serverchatlog.html", serverinfo=instanceinfo(instance), chatlog=chatlog[::-1]
    )
